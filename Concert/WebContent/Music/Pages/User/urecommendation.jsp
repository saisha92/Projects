...
PreparedStatement ps = connect.prepareStatement("select user_id from user_follows where follow_id in (select user_id from user_follows where follow_id = ?); ");
...
<div class="wrapper col4">
  <div id="container">
  <center><h2>RECOMMENDATIONS</h2>
  <center>
<table border=2>
	<tr>
		<th>You might probably be interested in these people</th>
	</tr>
	 <%
    while(rs.next())
    {
    ...
    %>
	<tr>
	<td><a href="<%=request.getContextPath() %>/MusicControl?redirect1=<%=rs.getString("user_id") %>" ><%=rs.getString("user_id") %></td>
	</td>
	</tr>
	<%} %>
	</table>
