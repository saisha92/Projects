...
User user = (User)request.getSession().getAttribute("userdetails");
  ...
  PreparedStatement ps2 = connect2.prepareStatement("Select * from user where user_id = ?");
...
News Feed<br>
 ...
 <iframe id="NewsWindow" seamless src="news_win.jsp" width="250" height="350" marginwidth="0" marginheight="0" frameborder="0" scrolling="no" style="display: block; margin: 0 auto; padding: 0; border: #000000 1px solid;"></iframe>
    ...
    <div class="fl_left">
    <h2>Welcome Back</h2>
      <h2><%=" "+rs2.getString("user_name")%></h2>
      <h3>Profile</h3>
      <div class="circle" style="background-image:url('<%=request.getContextPath()%>/Music/profilepic/<%=rs2.getString("image")%>')"></div>
      <form name="form1" method="post" enctype="multipart/form-data" action="<%=request.getContextPath() %>/MusicControl?action=uploadpic&uploadpicid=<%=user.getUid()%>">
      <p>
      <input type="file" name="ImageFile" id="ImageFile" />
      </p><p>
       <input type="hidden" name="uploadpicid" value=<%=user.getUid() %>></input>
      <input type="submit" name="Upload Picture" value="picupload" />
      </p>
      </form>
      <p >You are from : <%=""+rs2.getString("city") %> </p>
            <p>We will wish you on <%=""+rs2.getString("DOB") %></p>
            <p>About Self <%=""+rs2.getString("profile_details") %></p>
            <p>Your Current Trust Score : <%=""+rs2.getString("trustscore") %></p>
            <p>Joined Us on :<%=""+rs2.getString("date_joined") %></p>
            <p>Your Last Access : <%=""+rs2.getString("last_access_time") %></p>
    </div>
    <br class="clear" />
  </div>
</div>
