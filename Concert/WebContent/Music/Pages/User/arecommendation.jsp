...
PreparedStatement ps = connect.prepareStatement("select artist_id from fan_of where user_id in (select user_id from user_follows where follow_id = ?); ");
...
<table border=2>
	<tr>
		<th>You might probably be interested in these Artists</th>
	</tr>
	 <%
    while(rs.next())
    {
    ...
    %>
	<tr>
	<td><a href="<%=request.getContextPath() %>/MusicControl?aredirect=<%=rs.getString("artist_id") %>" ><%=rs.getString("artist_id") %></td>
	</td>
	</tr>
	<%} %>
	</table>
