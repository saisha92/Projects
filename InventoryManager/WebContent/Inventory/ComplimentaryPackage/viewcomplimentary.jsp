...
<%
	ArrayList<Product> StockList = (ArrayList)request.getSession().getAttribute("packagelist");
%> 
<%
 	if (StockList != null) {
...
<table border="1">
	...
	<%
		while (itr.hasNext()) {
				Compliment cObj = (Compliment) itr.next();
	%>
	<tr>
		<td><%=cObj.getPackageId()%></td>
		<td><%=cObj.getOffers()%></td>
	<%
		}
	%>
</table>
